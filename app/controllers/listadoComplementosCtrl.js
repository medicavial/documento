function listadoComplementosCtrl($scope, $rootScope, find ,loading,$filter,$location,$http,relaciones,api){
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
                    { field:'CMP_foliofiscal',displayName:'Folio Fiscal', width: 300, pinned: true},
                    { field:'CMP_monto',displayName:'Total', width: 150 , pinned: true, cellTemplate: '<div ng-class="{ \'text-danger\': row.entity.penalizado ==  \'1\'}" class="padding-cell"><i ng-if="row.entity.penalizado ==  \'1\'" class="glyphicon glyphicon-warning-sign"></i> {{row.getProperty(col.field)}}</div>'},
                    { field:'CMP_tipocomprobante',displayName:'Tipo Comprobante', width: 150, pinned: true },
                    { field:'CMP_fechaEmision',displayName:'Fecha Emisión', width: 150 , pinned: true},
                    { field:'CMP_rfcEmisor',displayName:'RFC Emisor', width: 150 },
                    { field:'CMP_nombreEmisor',displayName:'Nombre Emisor', width: 150 }
