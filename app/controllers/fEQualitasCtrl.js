...
function fEQualitasCtrl($scope, $rootScope, $filter, find , loading, checkFolios,datos,$timeout,facturacionExpress,webStorage,operacion,tickets,saceco,$http){
    ...
    $scope.listado = datos[4].data.listado;
    ...
    $scope.gridOptions = {
    	...
    	columnDefs: [
                    ...
                    { field:'QWS_folioAutorizacion', displayName:'Folio Autorizacion', width: 120, pinned:true, enableCellEdit: false },
    ...
    $scope.onDblClickRow = function(row){
        ...
        $scope.captura.triage = '';
        $scope.captura.MedicoMV = '';
        $scope.captura.RIEClave = '';
        $scope.datos.cliente=0;
        $scope.listDocmuento = '';
        $scope.motRechazo = '';
        $scope.anotaciones= '';
        $scope.estatusDocto='';
    ...
    //Aprobar documentos desde fuera sin ver la imagenes
    $scope.apruebaDocumento = function(tipoDocumento){
        var lista = $scope.listDocmuento;
        $scope.verCaptura=1;
        for (var i = lista.length - 1; i >= 0; i--) {
            if(lista[i].TID_clave==tipoDocumento){
                lista[i].Arc_estatus=1;
                lista[i].ATD_estatus=1;
            }
        }
        for (index = 0; index < lista.length; ++index) {
            value = lista[index].Arc_estatus;
            if(value==2||value==0){
                $scope.verGuardaTicket=1;
                $scope.verCaptura=2;
            }
        }
        $scope.listDocmuento=lista;
    }

    $scope.resetRadio = function(tipoDocumento){
        var lista = $scope.listDocmuento;
        for (var i = lista.length - 1; i >= 0; i--) {
            if(lista[i].TID_clave==tipoDocumento){
                lista[i].Arc_estatus=0;
                lista[i].ATD_estatus=0;
            }
        }
        $scope.listDocmuento=lista;
...
fEQualitasCtrl.$inject = ['$scope', '$rootScope', '$filter', 'find', 'loading', 'checkFolios','datos','$timeout','facturacionExpress','webStorage','operacion','tickets','saceco','$http'];

app.controller('fEQualitasCtrl',fEQualitasCtrl);
